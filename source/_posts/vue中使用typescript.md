...
* Cannot find module './xxx/xxx' or its corresponding type declarations
  原因是ts识别不了vue文件，在引入的最后加上`.vue`就可以了
  ~~~js
    // 错误的写法
...
* 获取DOM元素时提示  `Type 'HTMLElement | null' is not assignable to type 'HTMLElement'. Type 'null' is not assignable to type 'HTMLElement'`
  ...
  eg: 
  ~~~js
    this.scaleBox = document.querySelector('.scale-box')
  ~~~

  对获取到的信息做一次**类型转换**或者**类型断言**
  ~~~ts
    this.scaleBox = <HTMLDivElement>document.querySelector('.scale-box')
    this.scaleBox = document.querySelector('.scale-box') as HTMLElement
  ~~~


同时贴上我在网上找到的`tslint`配置
